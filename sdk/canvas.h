...
);  // true - не показывать статус-строку, false - показывать
...
//  рисует  линию
void UIS_CanvasDrawLine(GRAPHIC_POINT_T begin, GRAPHIC_POINT_T end, UIS_DIALOG_T handle);
...
void UIS_CanvasDrawPoly(UINT16 number_of_vertices, GRAPHIC_POINT_T *array, BOOL fill, UIS_DIALOG_T handle);
// рисует точку
...
#if (defined(FTR_L7E) || defined(FTR_L9))
UINT32 UIS_CanvasGetCharacterSize(WCHAR *str, UINT8 position, GRAPHIC_METRIC_T *char_size, FONT_ATTRIB_T font_id);
#else
UINT32 UIS_CanvasGetCharacterSize(WCHAR *str, UINT8 position, GRAPHIC_METRIC_T *char_size, UINT8 font_id);
#endif
...
UINT32 UIS_CanvasDrawImage(
	UINT8 picture_type,  // тип. откуда будем брать картинку. DRM, файл....
...
UINT32 UIS_CanvasDrawColorBitmap(
	void *picture_bytes,  // первый байт - ширина, второй байт высота
...
UIS_CanvasOpenImageAndGetSize(UINT8 picture_type, PICTURE_TYPE_UNION_T picture_data, GRAPHIC_POINT_T *attributes);
// закрывает картинку
...
// перемещает регион
UINT32 UIS_CanvasMoveRegion(GRAPHIC_REGION_T source_region, GRAPHIC_REGION_T dest_region, UIS_DIALOG_T handle);
...
void UIS_CanvasDrawColorScrollBar(
	UINT16 count_items,         // кол-во пунктов
...
UINT8 UIS_CanvasGetSoftkeyRegion(GRAPHIC_REGION_T *region, UINT8 type_softkey, BOOL use_soft_icon_area);

UINT8 UIS_CanvasGetSoftkeyTextRegion(GRAPHIC_REGION_T *region, UINT8 type_softkey, BOOL use_soft_icon_area);
