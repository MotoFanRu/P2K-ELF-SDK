...
// Возвращает "тип" верхнего диалога. В приложениях прошивки часто используется для определения нахождения на рабочем
// столе.
//  Если в res значение 0xE, значит на рабочем столе. Для обычного списка в res значение 0x10. Поэксперементируйте если
//  вам это нужно.
...
enum {
	...
	NOTICE_TYPE_WAIT,
	NOTICE_TYPE_WAIT_NO_KEY = NOTICE_TYPE_WAIT,  // кнопки блокируются, к контенту в конце добавляются точки, приложение
	                                             // должно само както вывести из стэйта кнопками можно вывести только
	                                             // вызвав метку (при этом закроется приложение)
...
UIS_DIALOG_T
UIS_CreateNotice(SU_PORT_T *port, CONTENT_T *content, UINT32 duration, UINT8 type, BOOL critical, ACTIONS_T *actions);
...
UIS_DIALOG_T UIS_CreateViewer(SU_PORT_T *port, CONTENT_T *contents, ACTIONS_T *actions);

/* Создает диалог вывода контента, где заголовок и тело задаются отдельно */
UIS_DIALOG_T
UIS_CreateViewerWithTitle(SU_PORT_T *port, CONTENT_T *title, CONTENT_T *body, ACTIONS_T *actions, RESOURCE_ID dlgres);
