 ...
 *    Если оба вида отладчиков будут выключены, то обьектый файл 'dbg.o' никак не будет влиять на размер конечного
 * приложения
 *
 *
 *  Макросы
 ...
 *      Форматированный вывод сообщений со пометкой 'err'(Ошибка) в midway. Используйте при неожиданных выходах из
 * программы 4) void * malloc(size_t size); Запрос к менеджеру памяти на выделение size байт оперативной памяти 5) void
 * free( void * mem ); Сообщение менеджеру памяти о том, что выделенная ранее память уже не нужна 6) void ddbg( const
 * void * data, size_t size ); Вывод в midway блока данных размером size, который расположен по адрессу data 7) void
 * udbg( const char * format, const WCHAR * ustr ); Форматированный вывод сообщения в midway с одной юникод строкой 8)
 * DEBUG_TIME ...
 *
 * ********************************************************************/
...
#define dmalloc(sz) dbg_malloc(app_name, sz, __FILE__, __LINE__)
#define dfree(mb)   dbg_free(app_name, mb, __FILE__, __LINE__)

#else  // DEBUG_MEMORY

#define dmalloc(sz) malloc(sz)
#define dfree(mb)   mfree(mb)
...
#define dbg(format)        TRACE("%s: " TRACE_DBG_STR "@%d: " format "\n", app_name, __LINE__)
#define dbgf(format, ...)  TRACE("%s: " TRACE_DBG_STR "@%d: " format "\n", app_name, __LINE__, ##__VA_ARGS__)
#define ddbg(d, sz)        dbg_data(app_name, d, sz, __FILE__, __LINE__)
#define udbg(format, ustr) dbg_unicode(app_name, "%s: " TRACE_U_STR "@%d: " format "\n", ustr, __FILE__, __LINE__)
...
#define dtime() dbg_time(app_name, __FILE__, __LINE__, dbg_timer_var)
