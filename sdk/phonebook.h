...
UINT32 DL_DbPhoneBookFindRecordByName(UINT8 list_type, WCHAR *name, UINT8 sort_type, UINT16 *index);

// поиск  записи по номеру телефона. Отправляет ивент EV_PHONEBOOK_READ_RECORD
UINT32 DL_DbPhoneBookFindRecordByTelno(IFACE_DATA_T *data, UINT8 list_type, WCHAR *tel_number, UINT8 sort_type);

// Кол-во возможных записей
UINT32 DL_DbPhoneBookGetNumberRecords(UINT8 list_type, PHONEBOOK_DEVICE_T device, UINT16 *count_numbers);
// кол-во используемых записей
UINT32 DL_DbPhoneBookGetUsedRecords(UINT8 list_type, PHONEBOOK_DEVICE_T device, UINT16 *count_numbers, UINT8 sort_type);
// кол-во доступных записей
UINT32 DL_DbPhoneBookGetAvailableRecords(UINT8 list_type, PHONEBOOK_DEVICE_T device, UINT16 *count_numbers);

// удаление записи
UINT32 DL_DbPhoneBookDeleteRecordByIndex(IFACE_DATA_T *iface_data, UINT8 list_type, UINT16 index, UINT8 sort_type);

// Удалить все записи
UINT32 DL_DbPhoneBookDeleteAllRecords(IFACE_DATA_T *iface_data, UINT8 list_type);

// обновить запись
UINT32 DL_DbPhoneBookUpdateRecord(IFACE_DATA_T *iface_data, UINT8 list_type, void *record, UINT8 sort_type);

// новая запись
UINT32 DL_DbPhoneBookStoreRecord(IFACE_DATA_T *iface_data, UINT8 list_type, void *record, UINT8 sort_type);
...
// удаление категории
UINT32
DL_DbPhoneBookDeleteCategoryRecord(IFACE_DATA_T *data, UINT8 list_type, PHONEBOOK_DEVICE_T device, UINT8 category_id);
