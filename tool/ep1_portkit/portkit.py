...
def bin2fpa(firmware, author, description, address, binary):
	if '.bin' in binary:
		...
		with open(binary, 'rb') as input, open(fpa, 'w') as output:
			...
			output.write('; This patch file was generated by bin2fpa.py\r\n')
			output.write('[Patch_Info]\r\n')
			output.write(f'SW_Ver={firmware}\r\n')
			output.write(f'Author={author}\r\n')
			output.write(f'Description={description}\r\n')
			output.write('[Patch_Code]\r\n')
			output.write(f'{addr}:{hex_data}\r\n')
	else:
...
def start_portkit_routines(ram_trans_flag, base_address, patterns, cg1):
	...
	# Generate patch for Flash & Backup 3 from ElfPack.
